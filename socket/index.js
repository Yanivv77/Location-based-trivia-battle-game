...
io.on('connection', (socket) => {
  ...
  socket.on('startGame', (gameId, callback) => {
    ...
    if (gameId) {
      ...
      console.log(`player: ${players[0].name},total players: ${players.length}`)
  ...
  socket.on('getNextQuestion', (roomId) => {
    ...
    if (remaining === 0) {
      ...
      if (waitingPlayers.length > 0) {
        ...
        const players = GameManager.getPlayersByRoom(roomId)
        console.log(`Total players after adding waiting:${GameManager.players.length} `)
    ...
    if (player) {
      ...
      if (waiting === 0) {
        ...
        if (remaining === 0) {
          ...
          let waitingPlayers = GameManager.getWaitingPlayersByGame(player.roomId)
          console.log('waiting players', waitingPlayers)
